...
The report is also published to GitHub Pages, so it opens in the browser without downloading the artifact: `scripts/github-actions/publish-report-pages.py` pushes every run to the `gh-pages` branch under `runs/<run id>/`, keeps the last 30 runs and regenerates the site index at https://dmitry-minchuk.github.io/java-taf-template/ with the date, build, results and tests commit of each run; the job summary links to the published report. The site is public, like the repository.
