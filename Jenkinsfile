...
def functionalJobList = [new Job("sample_ui_suite", "webstudio:latest", "", ""),
                         new Job("sample_ui_suite", "webstudio:latest", "", ""),
                         new Job("sample_ui_suite2", "webstudio:latest", "", jenkinsLabel.slave2SAML.nodeLabel)]
...
pipeline {
    ...
    parameters {
        string(name: 'APPLICATION_GIT_COMMIT_HASH_VERSION', defaultValue: 'xxxxxxx', description: 'Source application branch (openl-tablets)')
                        ...
                        [(suite): {
                            node(suite.nodeToRunOn) {
                                ...
                                withMaven() {
                                    ...
                                    sh("bash -lc 'mvn clean test -Dsuite=${suite.suiteName} -Dcontainer.app.path=${suite.containerAppPath} -Dimage.name=${suite.imageName} -Dtestng.dtd.http=true'")
                                }
