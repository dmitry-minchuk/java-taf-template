                        ...
                        [(suite): {
                            node(suite.nodeToRunOn) {
                                ...
                                withMaven() {
                                    ...
                                    sh("bash -lc 'mvn clean test -Drp.endpoint=http://10.23.172.185:8080 -Drp.project=OpenL_Tests -Drp.launch=${suite.suiteName} -Drp.uuid=${RP_UUID} -Dsuite=${suite.suiteName} -Dcontainer.app.path=${suite.containerAppPath} -Dimage.name=${suite.imageName} -Dtestng.dtd.http=true'")
                                }
